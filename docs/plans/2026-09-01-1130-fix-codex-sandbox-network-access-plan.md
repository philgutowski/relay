...
- The grant is emitted for every `codex` Task, whatever the tracker adapter. A `markdown` adapter run needs no network at all and gets the reach anyway. Making the emit conditional on the adapter would move a launch fact out of the pin, so the pin comment states the grant is unconditional instead.

#### Out of scope, named because a reviewer raised it

- Sharpening the halt an operator sees when `--strict-config` refuses a key. The Cause line would read `findings_unavailable`, about evidence that could not be read, when the real cause is a rejected config; the stdout log carries codex's own error line either way. The fix is a launch rejection branch in `run._one_task` ahead of that halt, reusing `HALT_UNEXPECTED_ERROR` so the closed set stays closed. It is a Runner halt path change, which is a different piece of work from removing the fence.
...
- KTD6. **Carry `--strict-config` on the `codex` argv, accepting that it also validates the operator's `~/.codex/config.toml`.** Without it, an override key the CLI no longer recognizes is accepted and ignored: the process runs, the sandbox stays fenced, and the first symptom is the blocked halt plus the hand landing this plan removes. Neither the suite nor the stub can see that, because both derive the argv from the same pin. With it, the same key produces `Error loading config.toml: unknown configuration field` and the process never starts. The Runner does halt on that, but not with the message a reader would expect: `launch.launch` sets `launch_error` only inside its `except OSError`, so a process that spawns and then exits non zero falls through to the `findings_unavailable` halt, whose text is about evidence that could not be read and restrictions that were not audited. The stdout log holds codex's own error line, so the halt is diagnosable, but the Cause line points at the wrong thing. Sharpening it is deferred, see Scope Boundaries. The other cost is that `--strict-config` also validates the operator's own `~/.codex/config.toml`, so a field this `codex` version does not recognize there would fail every launch. That is loud rather than silent, and the operator's current config passes today. Governs R9.
...
- **Files:**
  ...
  - `tests/test_stub.py`
- **Approach:**
  1. In `backends.codex.build_args`, append `--strict-config` when `cap.strict_config`, then extend the argument list with `-c` plus the token once per entry in `cap.config_overrides`, after the `--add-dir` loop and before the brief is appended. The brief has to stay the last positional; nothing else about the order matters, and the landed order is the one the live probe exercised.
...
- The exact argv `backends.codex.build_args` emits, `--add-dir <repo>/.git` and `--strict-config` and `-c sandbox_workspace_write.network_access=true` all together: a write under `.git/` exited 0 and `gh api user` exited 0 in the same process. So the `-c` override does not replace the `sandbox_workspace_write` table and cost the writable root `--add-dir` supplies. A code review raised that as the one plausible way this change could break committing, which is why it is pinned here.
