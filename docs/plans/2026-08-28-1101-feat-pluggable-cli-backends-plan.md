...
- **A backend that cannot enforce a restriction at launch carries it in the Brief and records it as unenforced.** (session-settled: user-approved, chosen over holding Codex until it gains launch-time denial: Verify-landed and the Lease were always the real guards, and a misbehaving process is caught the same way regardless of backend.) Governs R10.
...
- **What a backend cannot refuse at launch is gated at authoring, bounded at the landing, and audited afterwards.** This is a plan-time addition, not part of the settled decision above: the operator settled on carrying the restriction in the Brief and recording it, and this adds three compensating controls on top. Recording a gap and requiring the operator to accept it in writing are different acts, so the acceptance sentence belongs here rather than to the settled decision. Governs R19, R21, R24.
...
R12. Verify-landed continues to read git and the Tracker alone, so a landing verdict never depends on which backend did the work. This requirement is satisfied by leaving `verify.py` untouched, so no unit realizes it and no unit may weaken it. U14's parity partition is where it is observed to still hold.
...
KTD16. **The run-level environment and the child's copy are narrowed differently, and both narrowings are mandatory.** `child_env` is built once per run before any Task is in scope, and that same environment is what the git operations, the Tracker adapters, and the version probes use, so it keeps every backend's credentials and scrubs the union of every backend's nesting markers. `launch.launch` then narrows the child's copy to the launching Task's own backend credentials, dropping the others. Unknown bare variable names are not scrubbed; the manifest is where an operator adds one, the way `tracker.token_env` already works. Rationale: a run-level scrub alone can only be all-or-nothing, so it either strips the running Task's own credentials or isolates nothing, and the per-Task narrowing is the only thing that satisfies R23. This entry is the single owner of that split; the Assumptions, System-Wide Impact, and U5 cite it rather than restating it. Governs R23.
...
- The two environment narrowings are owned by KTD16. Both are mandatory, and `child_env` is called at `run.py:121` before any Task exists, which is why the per-Task narrowing has to happen inside `launch.launch` rather than there.
...
4. **Split the two environment treatments per KTD16.** `child_env` gains an optional backend argument: `run.py:121` passes none, `launch.launch` passes the Task's backend. The variable names and prefixes come from the capability record, observed in U1.
...
- **Two environments are narrowed differently and both matter (KTD16).** Getting either wrong fails silently in opposite directions: too much scrubbing and a Task cannot authenticate, too little and a Task process holds a credential it should never see. Neither failure is visible in a test that does not know the variable names, which is why U1 observes them and U4 records them.
