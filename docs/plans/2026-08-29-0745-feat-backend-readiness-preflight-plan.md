...
- **Means:** Extend manifest validation with capability-record-driven environment readiness checks and adapter/backend compatibility checks. (KTD1, KTD2)
...
- Deferred to follow-up work: building backend argv, child-environment treatment, and runtime version recording belong to the parent plan's U5.
...
- Missing binaries, missing or too-old plugins, and incompatible adapter/backend pairs are separately diagnosable.
