...
- **Execution profile:** single-repo, Python standard library, no new process or external dependency. Verified entirely against the existing stub-`claude` test suite; no live run required (see Verification Contract).
...
- KTD3. **`run()`'s per-task except handler consults the opt-in, the class, and the disposition before writing the terminal record.** After the three existing `except` branches settle `halt`: when `manifest.on_halt.continue_past_task_halt` is set and `halt.halt_class not in contracts.RUN_SCOPED_HALT_CLASSES`, call `gitwrite.resume_disposition`. When it allows, keep the existing `store.upsert(..., status=STATUS_HALTED, ...)` call, add `continued_past=True` to it, then `store.set_cursor(index + 1)` and `continue`. Every other case, opted out, run-scoped, or disposition refused, keeps today's stop-write-terminal-return path unchanged; when the disposition refused, its failed check and evidence are merged into `halt_evidence` under a `resume` key so the record says why the run did not continue. The disposition runs inside the same `try` discipline as the rest of the handler: a `GitError` from it is a stop with today's `HALT_UNCLEAN_EXIT` wrapping, never an escape. Governs R1, R2.
...
flowchart TD
    ...
    C -- yes --> S
    C -- no --> D["gitwrite.resume_disposition(repo, default)\ndirty tree? refuse\nnot on default? checkout default\nhead != origin/default? refuse"]
    D -- refused --> S2["merge failed check into halt_evidence['resume']\nthen same as S"]
...
What the disposition decides for each halt the runner can raise today, so the implementer can check the mechanism against the raise sites rather than trust a list:
...
| `_one_task`, preflight refused | `unclean_exit` | whatever preflight refused | refuse (same check) |
| `_one_task`, complete with no commits | `unclean_exit` | on default, clean | allow |
...
| `_run_closeout`, scope check | `closeout_out_of_scope` | reset to pushed pre-closeout head | allow |
| `run()`, `GitError` catch-all | `unclean_exit` | unknown | disposition decides |

`_one_task`, `_timeout_route`, `_merge_route`, `_blocked_route`, `_run_closeout`, and `verify.startup_reverify` are unchanged: they already raise `_Halt` the same way, and a `STATUS_HALTED` record they leave behind is already retried on the next `relay run` (Open Questions, item 2).
...
- No live run against the throwaway proof target is required. This change is runner-internal control flow plus one new git read sequence built from calls `gitwrite.preflight` and `blocked_path` already make; it does not touch the envelope grammar, the closeout terminal line, a brief template, the halt record's evidence shape, or the classify digest keys, the four contract surfaces `CLAUDE.md` requires a live run for.
...
- `docs/solutions/logic-errors/cause-line-contract-split-degraded-to-placeholders.md`, why this plan changes no `HALT_LINES` template or evidence shape (Scope Boundaries): that contract has already broken silently once from an uncoordinated edit.
...
**Approach:**
1. In `run()`'s task loop, after the three `except` branches settle `halt` and before the existing `store.upsert` / `write_terminal` / `return` sequence, insert the KTD3 decision. Keep the decision in one small helper (`_may_continue(cfg, halt)` or similar) that returns the disposition result or `None`, so the loop body reads as the sequence in the diagram.
2. When allowed: the existing `store.upsert(...)` call gains `continued_past=True`; then `store.set_cursor(index + 1)` and `continue`. Do not call `write_terminal`; do not return.
3. When refused: merge `{"resume": {"check": result.failed, **result.evidence}}` into `halt.evidence` before the existing upsert, then fall through to today's stop path unchanged.
4. When opted out or run-scoped: fall through unchanged, no disposition call, no git activity.
5. A `GitError` raised by the disposition itself takes the existing `HALT_UNCLEAN_EXIT` wrapping the loop already applies to `_one_task`, with the original halt's class and message preserved in the evidence so the operator sees both.
...
- Resume after a continued halt: mirroring `ResumeAfterHalt`, a second `run()` after a continued-past gate refusal retries that task through the existing path and lands it, with no new flag; the record's `continued_past` is cleared by the landing upsert (`halt_class=HALT_LANDED`, `continued_past=False` or removed).
...
2. In `_pending_checks`, beside the existing `STATUS_BLOCKED` branch: when `entry["status"] == STATUS_HALTED and entry["continued_past"]`, append `{"kind": "continued_past", "task": task_id, "text": "%s halted with class %s and the run continued past it. Repair by hand, then run again to resume." % (task_id, entry["class"])}`.
...
- No live run against the throwaway proof target is required (Assumptions).
